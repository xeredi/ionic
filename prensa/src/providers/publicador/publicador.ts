...
export class PublicadorProvider extends SqliteProvider {
    ...
    dropTable() {
        this.open()
            .then(() => {
                this.db.executeSql( 'DROP TABLE IF EXISTS publicador', [] )
                    .then( data => data )
                    .catch( error => { console.log( error ); } );
            } )
            .catch( error => { console.log( error ); } );
    ...
    save( item: any ) {
        this.open()
            .then(() => {
                this.db.executeSql( 'SELECT 1 FROM publicador WHERE url = ? ', [item.url] )
                    .then( data => {
                        ...
                        if ( data.rows.length <= 0 ) {
                            this.db.executeSql(
                                ...
                                , [item.url, item.logo, item.nombre, item.tipo, item.categoria] )
                                .then( data => data )
                                .catch( error => { console.log( error ); } );
                        }
                    } )
