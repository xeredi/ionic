...
export class CategoriaProvider extends SqliteProvider {
    ...
    dropTable() {
        this.open()
            .then(() => {
                this.db.executeSql( 'DROP TABLE IF EXISTS categoria', [] )
                    .then( data => data )
                    .catch( error => { console.log( error ); } );
            } )
            .catch( error => { console.log( error ); } );
    ...
    save( item: any ) {
        this.open()
            .then(() => {
                this.db.executeSql( 'SELECT 1 FROM categoria WHERE nombre = ?', [item.nombre] )
                    .then( data => {
                        // console.log( "exists: " + ( data.rows.length > 0 ) );
                        if ( data.rows.length > 0 ) {
                            this.db.executeSql( 'UPDATE categoria SET icono = ? WHERE nombre = ?', [item.icono, item.nombre] )
                                .then( data => data )
                                .catch( error => { console.log( error ); } );
                        } else {
                            this.db.executeSql( 'INSERT INTO categoria (icono, nombre) VALUES (?, ?)', [item.icono, item.nombre] )
                                .then( data => data )
                                .catch( error => { console.log( error ); } );
                        }
                    } )
